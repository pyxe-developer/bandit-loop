...
This reference preserves the Codex Bandit dispatch and identity contracts for
protocol compatibility. The Pi package does not install Codex TOML agents under
`~/.codex/agents`; Pi runs this workflow in assisted mode unless a separate,
Pi-native role runner is configured by the user.
